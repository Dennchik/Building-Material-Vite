export function counterProduct() {
  const counters = document.querySelectorAll('[data-quantity-container]');
  ...
  function _totalSum() {
    ...
    productPrices.forEach((productPrice) => {
      ...
      let valuePrice =
        parseFloat(
          productPrice.textContent.replace(/ /g, '').replace(',', '.')
        ) || 0;
      let quantity =
        parseFloat(
          productPrice.closest('.product-cart').querySelector('input').value
        ) || 0;
      ...
      productPrice
        ...
        .querySelector('.product-cart__quantity-price').textContent =
        totalSumProduct.toLocaleString('ru-RU', {
          style: 'currency',
          currency: 'RUB',
          minimumFractionDigits: 0,
        });
      inputTotal.innerHTML =
        'Итого:' + ' ' + totalSum.toLocaleString() + `<i class="icon-rub"></i>`;
    });
  }
